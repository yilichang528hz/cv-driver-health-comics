...
新版閱讀器採用閱讀優先設計：進入角色設定或任一漫畫主題後，頂部導覽會自動收合為一條薄型控制列；再次點選「展開導覽」即可向下展開完整首頁橫幅控制板。

閱讀器支援：

- 上一張、下一張
- 預設適合寬度顯示，讓漫畫頁面放大至閱讀區寬度
- 縮小、100%、放大、適合寬度
- 沉浸閱讀與返回一般閱讀
- ESC 離開沉浸閱讀
- 左右方向鍵換頁
- `+` / `-` 縮放，`0` 回到 100%，`F` 切換沉浸閱讀
- 雙擊圖片可在 100% 與 160% 間切換
- 手機與平板 RWD
- CSS 沉浸式全視窗閱讀模式，不強制呼叫瀏覽器 Fullscreen API，因此仍可保留瀏覽器縮放與頁面互動彈性
- 僅預載相鄰頁，不會在首頁一次載入全部漫畫圖片

首頁狀態下，上一張、下一張與縮放控制會停用；主題或角色設定狀態下，按鈕會依頁碼與縮放倍率自動啟用或停用。
