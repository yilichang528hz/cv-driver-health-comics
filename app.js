...
let readerZoomLevel = 1;

const READER_ZOOM_LEVELS = [0.75, 0.9, 1, 1.15, 1.35, 1.6, 2, 2.5, 3];
...
const elements = {
  ...
  readerChromeToggleBtn: $("readerChromeToggleBtn"),
...
function setChromeCollapsed(nextCollapsed) {
  const canCollapse = currentView !== "home";
  chromeCollapsed = Boolean(nextCollapsed && canCollapse);
  elements.appShell.classList.toggle("chrome-collapsed", chromeCollapsed);
  elements.readerChromeToggleBtn.hidden = !canCollapse;
  elements.readerChromeToggleBtn.setAttribute("aria-expanded", String(!chromeCollapsed));
  elements.readerChromeToggleBtn.textContent = chromeCollapsed ? `${t("expandHeader")} ▾` : `${t("collapseHeader")} ▴`;
}

function updateViewChromeState({ reading, collapse } = {}) {
  const isReading = Boolean(reading);
  elements.appShell.classList.toggle("is-reading-mode", isReading);
  elements.appShell.classList.toggle("is-home-mode", !isReading);
  setChromeCollapsed(isReading ? collapse : false);
}
...
function applyReaderZoom() {
  const isReaderImage = currentView !== "home";
  elements.readerShell.dataset.zoom = String(Math.round(readerZoomLevel * 100));

  if (!isReaderImage) {
    elements.ebookImage.style.width = "";
    elements.ebookImage.style.maxWidth = "";
    elements.ebookImage.style.maxHeight = "";
    elements.ebookImage.style.cursor = "";
    setZoomControlsDisabled(true);
    return;
  }

  elements.ebookImage.style.width = `${Math.round(readerZoomLevel * 100)}%`;
  elements.ebookImage.style.maxWidth = "none";
  elements.ebookImage.style.maxHeight = "none";
  elements.ebookImage.style.cursor = readerZoomLevel > 1 ? "grab" : "zoom-in";
  setZoomControlsDisabled(false);
}

function setZoomControlsDisabled(disabled) {
  [elements.zoomOutBtn, elements.zoomResetBtn, elements.zoomInBtn, elements.fitWidthBtn].forEach((button) => {
    button.disabled = disabled;
  });
  elements.zoomOutBtn.disabled = disabled || readerZoomLevel <= READER_ZOOM_LEVELS[0];
  elements.zoomInBtn.disabled = disabled || readerZoomLevel >= READER_ZOOM_LEVELS[READER_ZOOM_LEVELS.length - 1];
}

function zoomBy(step) {
  if (currentView === "home") return;
  let index = READER_ZOOM_LEVELS.findIndex((level) => level >= readerZoomLevel - 0.001);
  if (index < 0) index = 2;
  index = Math.max(0, Math.min(READER_ZOOM_LEVELS.length - 1, index + step));
  readerZoomLevel = READER_ZOOM_LEVELS[index];
  applyReaderZoom();
}
...
function fitWidth() {
  ...
  readerZoomLevel = 1;
  applyReaderZoom();
  elements.ebookFrame.scrollTo({ top: 0, left: 0, behavior: "smooth" });
}
...
function renderHome(expand = motivationExpanded) {
  ...
  updateViewChromeState({ reading: false, collapse: false });
...
function renderCharacters() {
  ...
  currentImageIndex = 0;
  resetReaderZoom();
  updateViewChromeState({ reading: true, collapse: true });
...
function setLanguage(lang) {
  ...
  elements.fullscreenBtn.textContent = t("immersiveMode");
  elements.exitFullscreenBtn.textContent = t("exitImmersiveMode");
  elements.zoomOutBtn.textContent = t("zoomOut");
  elements.zoomResetBtn.textContent = t("resetZoom");
  elements.zoomInBtn.textContent = t("zoomIn");
  elements.fitWidthBtn.textContent = t("fitWidth");
  setChromeCollapsed(chromeCollapsed);
...
function enterFullscreen() {
  ...
  elements.appShell.classList.add("is-immersive-mode");
  if (currentView !== "home") setChromeCollapsed(true);
...
async function exitFullscreen() {
  try {
    ...
    // CSS immersive mode still exits below.
  }
  elements.readerShell.classList.remove("is-fullscreen");
  elements.appShell.classList.remove("is-immersive-mode");
  updateFullscreenState();
}
...
function updateFullscreenState() {
  ...
  elements.appShell.classList.toggle("is-immersive-mode", active);
...
function bindEvents() {
  ...
  elements.readerChromeToggleBtn.addEventListener("click", toggleChrome);

  elements.prevPageBtn.addEventListener("click", goPrevPage);
  elements.nextPageBtn.addEventListener("click", goNextPage);
  elements.zoomOutBtn.addEventListener("click", () => zoomBy(-1));
  elements.zoomResetBtn.addEventListener("click", resetReaderZoom);
  elements.zoomInBtn.addEventListener("click", () => zoomBy(1));
  ...
  elements.ebookImage.addEventListener("dblclick", () => {
    if (currentView === "home") return;
    readerZoomLevel = readerZoomLevel > 1 ? 1 : 1.6;
    applyReaderZoom();
  });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if ((event.key === "f" || event.key === "F") && !event.metaKey && !event.ctrlKey && !event.altKey) {
      elements.readerShell.classList.contains("is-fullscreen") ? exitFullscreen() : enterFullscreen();
    }
    if (event.key === "Escape") {
      exitFullscreen();
      closeTopMenus();
